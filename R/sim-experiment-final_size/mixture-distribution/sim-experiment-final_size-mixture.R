...
for(i in 1:n_models){
  print(i)
  dat_filt = error_df %>% filter(model_id == paste0("M", i))
  preds_all[[i]] = fit_mixture_w_boxcox(
    dat_filt, new_vax_cov, alphas, plot_flag = TRUE, plot_title = paste("Model:", i)) %>% 
...
p1 = bind_rows(preds_all) %>% 
  ...
  geom_point(data = error_df, aes(y = error), shape = 21) +
...
p2 = ggplot(data = fits_var_small, aes(x = alpha, y = cov, group = model_id)) + 
  geom_line(aes(color = model_id), size = 1, alpha = 0.7) + 
  geom_abline(size = 1) + 
...
cowplot::plot_grid(p1, p2, rel_widths = c(0.7, 0.3))
ggsave("R/sim-experiment-final_size/mixture-distribution/500locations_lowcorr.pdf", width = 14, height = 6)

#### REPEAT WITH FEWER LOCATIONS -----------------------------------------------
# run simulation - now using 50 locations
...
preds_all_small <- vector("list", length = n_models)
for(i in 1:n_models){
  print(i)
  dat_filt = error_df_small %>% filter(model_id == paste0("M", i))
  preds_all_small[[i]] = fit_mixture_w_boxcox(
    dat_filt, new_vax_cov, alphas, plot_flag = TRUE, plot_title = paste("Model:", i)) %>% 
    mutate(model_id = paste0("M", i))
}

p1 = bind_rows(preds_all_small) %>% 
  ggplot(aes(x = vax_cov)) + 
  # geom_point(data = t_large$errors %>% filter(scenario_id %in% c("S1", "S2")),
  #            aes(y = error), color = "darkgray", shape = 8) +
  # geom_line(data = t_large$errors %>% filter(scenario_id == "E"),
  #           aes(y = error, group = location_id), color = "darkgray", alpha = 0.25) +
  geom_point(data = error_df_small, aes(y = error), shape = 21) +
  geom_ribbon(aes(ymin = Q25, ymax = Q975, fill = model_id), alpha = 0.4) +
  geom_ribbon(aes(ymin = Q250, ymax = Q750, fill = model_id), alpha = 0.6) +
  geom_line(aes(y = Q500, color = model_id), size = 1) + 
  facet_wrap(vars(model_id)) + 
...
fits_var_small = calculate_coverage(
  # do some reshaping to match expected format for quant_fits
  quant_fits = bind_rows(preds_all_small) %>% 
    melt(c("model_id", "vax_cov")) %>% 
    filter(variable != "mean") %>%
    mutate(quantile = as.integer(gsub("Q", "", variable))/1000) %>%
    dplyr::select(-variable), 
