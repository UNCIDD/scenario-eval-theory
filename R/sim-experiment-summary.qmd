...
# fit GAM with cubic spline for mu and sigma
 gam_fit <- gamlss(
   error ~ cs(vax_cov), sigma.formula = ~ cs(vax_cov), tau.formula = ~ cs(vax_cov), 
   data = dat_filt,  family = ST1, trace = FALSE,
  mu.start = mean(dat_filt$error), sigma.start = sd(dat_filt$error), nu.fix = TRUE, nu.start = 10, tau.start = 1
 )
 
g_pls <- centiles.pred(gam_fit, type = "centiles", 
                          cent = seq(0.01,0.99,0.01)*100,
                          xvalues = seq(0.3, 0.45, length.out = 100), xname = "vax_cov", 
                          data = dat_filt)
gam_sum = reshape2::melt(g_pls, c("x")) %>%
      mutate(quantile = rep(seq(0.01,0.99,0.01),length(seq(0.01,0.99,0.01)) + 1)) %>%
      rename(vax_cov = x) %>%
      select(-variable)

ggplot() +
  geom_hline(yintercept = 0) +
  geom_line(data = t_large$errors,
            aes(x = vax_cov, y = error, group = location_id), color = "gray", alpha = 0.6) +
