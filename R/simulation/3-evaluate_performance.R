...
# load estimates
all_ests_across_locs = readRDS("output/simulation/estimated_errors_across_locations.rds") 
all_ests_all_locs = readRDS("output/simulation/estimated_errors_location_specific.rds")

true_errors = readRDS("output/simulation/sim_out.rds")$errors
...
for(i in 1:nrow(method_comparsion_results)){
  ...
  tmp_df = all_ests_across_locs %>% filter(scenario_id == tmp_scenario_id, model_id == tmp_model_id, approach == tmp_method)
  # get samples
  tmp_df = tmp_df %>%
  ...
  # get true error
  tmp_truth = true_errors %>% filter(scenario_id == tmp_scenario_id, model_id == tmp_model_id)
  # perform KS test
...
saveRDS(method_comparsion_results, "output/simulation/performance_evaluation_results_across_locations.rds")
...
saveRDS(method_comparsion_results, "output/simulation/performance_evaluation_results_location_specific.rds")
