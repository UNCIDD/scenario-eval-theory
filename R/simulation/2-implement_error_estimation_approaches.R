...
# step 2: calculate error by subtracting observation from projection in this scenario
...
approach2_obs_nocovariate = get_gam_PIs(gam_obs, xvals = data.frame(vax_cov = new_vax_cov)) %>%
...
approach2_samp_nocovariate = approach2_obs_nocovariate %>%
...
approach2_nocov_errors = approach2_samp_nocovariate %>% 
...
approach2_nocov_errors_all_locs = approach2_nocov_errors %>% 
  reframe(quantile = quantiles, 
          value = quantile(est_error, quantiles), .by = c("vax_cov", "scenario_id", "model_id", "location_id"))

approach2_nocov_errors_across_locs = approach2_nocov_errors %>% 
  reframe(quantile = quantiles, 
...
approach2_obs_covariate <- vector("list", length(alphas))
for(i in 1:nrow(pred_mat_covar)){
  approach2_obs_covariate[[i]] =  get_gam_PIs(gam_obs_covar, xvals = pred_mat_covar[i, -2]) %>%
    mutate(location_id = pred_mat_covar[i, "location_id"])
}

approach2_obs_covariate = bind_rows(approach2_obs_covariate) %>%
  rename(vax_cov = xval.vax_cov, location_R0 = xval.location_R0) %>% 
  mutate(scenario_id = ifelse(vax_cov == new_vax_cov[1], "S1", "S2"))

# step 2: calculate error
approach2_samp_covariate = approach2_obs_covariate %>%
  reframe(est_obs_samp = get_samps(quantile, value, 1e4),
          draw_id = 1:1e4, .by = c("vax_cov", "location_id")
  )

approach2_cov_errors = approach2_samp_covariate %>% 
  filter(vax_cov %in% c(0.3, 0.5)) %>%
...
approach2_cov_errors_all_locs = approach2_cov_errors %>% 
  reframe(quantile = quantiles, 
          value = quantile(est_error, quantiles), .by = c("vax_cov", "scenario_id", "model_id", "location_id"))

approach2_cov_errors_across_locs = approach2_cov_errors %>% 
  reframe(quantile = quantiles, 
          value = quantile(est_error, quantiles), .by = c("vax_cov", "scenario_id", "model_id"))
...
approach3_nocov_errors_across_locs = bind_rows(gam_errors_nocov, .id = "model_id") %>%
...
approach3_obs_covariate <- vector("list", length(n_models))
for(i in 1: n_models){
  ...
  approach3_obs_covariate[[i]] = bind_rows(tmp) %>%
    mutate(model_id = paste0("M", i))
}

# estimated errors for each location
approach3_cov_errors_all_locs = bind_rows(approach3_obs_covariate) %>%
  rename(vax_cov = xval.vax_cov, location_R0 = xval.location_R0) %>% 
  mutate(scenario_id = ifelse(vax_cov == vax_scenarios[1], "S1", ifelse(vax_cov == vax_scenarios[2], "S2", "E")))

# distribution of errors across locations
approach3_cov_errors_across_locs = approach3_cov_errors_all_locs %>%
...
all_ests_across_locs = approach1_errors %>%
  ...
  # add approach 2
  bind_rows(
      approach2_cov_errors_across_locs %>% mutate(approach = "2-covariates")
  ) %>%
  bind_rows(
    approach2_nocov_errors_across_locs %>% mutate(approach = "2-nocovariates")
  ) %>%
  # add approach 3
  bind_rows(
